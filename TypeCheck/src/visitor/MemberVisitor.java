 ...
 * <p>在这一次遍历中可以查出的类型错误有：
 * <ul>
 * <li>类型未定义：所继承的父类未找到/变量或方法返回值的类型名未找到
 * <li>变量重复定义：同一个类/同一个方法中的变量重复定义
 * <li>方法重复定义：同一个类中的方法重复定义
 * </ul>
 *
...
public class MemberVisitor extends GJDepthFirst<Object, MIdentifier> {
	...
	public Object visit(final MainClass n, final MIdentifier argu) {
		MMethod main = new MMethod(n.f6.tokenImage, n.f6.beginLine, n.f6.beginColumn);
		MIdentifier id = (MIdentifier) n.f11.accept(this, null);
		MVar arg = new MVar(id.getName(), id.getRow(), id.getColumn(), MType.OtherType);
		main.addArgument(arg);
	...
	public Object visit(final VarDeclaration n, final MIdentifier argu) {
		...
		MVar var = new MVar(name.getName(), name.getRow(), name.getColumn(), type);
	...
	public Object visit(final MethodDeclaration n, final MIdentifier argu) {
		...
		MMethod method = new MMethod(name.getName(), name.getRow(), name.getColumn());
	...
	public Object visit(final Identifier n, final MIdentifier argu) {
		if (argu == MIdentifier.TAG_CLASS) {
			...
			return new MIdentifier(n.f0.tokenImage, n.f0.beginLine, n.f0.beginColumn);
	}
